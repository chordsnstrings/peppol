...
const STEPS = [
  { icon: FileCheck2, label: "Validated · PINT AE UBL", detail: "0 errors" },
  { icon: Radio, label: "Transmitted · Peppol network", detail: "0235:100…" },
  { icon: ShieldCheck, label: "Reported to the FTA", detail: "TDD accepted" },
];

/**
 * The proof: an invoice being validated and transmitted to the FTA, with
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        style={{ background: "var(--ink-2)", borderColor: "var(--ink-line)" }}
      >
        {/* window bar */}
        <div className="flex items-center gap-2 border-b px-4 py-2.5" style={{ borderColor: "var(--ink-line)" }}>
          ...
          <span className="mkt-mono ml-2 text-[11px]" style={{ color: "var(--on-ink-faint)" }}>
            arks · transmit
          </span>
          <span
            className="mkt-mono ml-auto rounded-md px-2 py-0.5 text-[10px]"
            style={{ background: "var(--signal-ink)", color: "var(--signal-2)" }}
          >
            {active >= 3 ? "COMPLETED" : active === 0 ? "READY" : "SENDING"}
          </span>
        </div>
        ...
        <div className="px-5 pt-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mkt-display mt-0.5 text-lg font-bold">INV-2026-00042</p>
              <p className="mt-1 text-sm" style={{ color: "var(--on-ink-soft)" }}>
                to Gulf Materials Trading LLC
              </p>
            </div>
            <div className="text-right">
              <p className="mkt-mono text-2xl font-bold tabular-nums">AED 12,600.00</p>
              <p className="mkt-mono mt-0.5 text-[11px]" style={{ color: "var(--on-ink-faint)" }}>
                incl. VAT 5%
              </p>
            </div>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium" style={{ color: lit ? "var(--on-ink)" : "var(--on-ink-soft)" }}>
                    {s.label}
                  </p>
                </div>
                {lit && (
        ...
        >
          <span className="mkt-mono text-[11px]" style={{ color: "var(--on-ink-faint)" }}>
            Evidence bundle: UBL · TDD · timeline
          </span>
          <span className="flex items-center gap-3">
            <StatusTick label="DELIVERED" on={active >= 3} />
            <StatusTick label="REPORTED" on={active >= 3} />
          </span>
        </div>
      </div>
    </div>
  ...
  return (
    <span className="mkt-mono flex items-center gap-1 text-[11px]" style={{ color: on ? "var(--signal-2)" : "var(--on-ink-faint)" }}>
      <span
        ...
        style={{ background: on ? "var(--signal)" : "var(--ink-3)", color: "var(--signal-ink)" }}
      >
