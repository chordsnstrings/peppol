...
export async function relatedPartyNote(opts: {
  ...
  const [declared, comp, attestation, counterparties] = await Promise.all([
  ...
  const unassessed = counterparties
    .filter((c) => !declaredKeys.has(partyKeyOf(c.code)) && !declaredKeys.has(partyKeyOf(c.name)))
    .map((c) => c.name)
    .sort();
