...
d("statement of changes in equity and the notes", () => {
  ...
  describe("the matrix", () => {
    it("has the four equity accounts as its columns, named from the chart", async () => {
      const s = await E();
      expect(s.columns.map((c) => c.code)).toEqual(["3000", "3100", "3200", "3900"]);
      expect(s.columns[0].name).toBe("Share capital");
      expect(s.columns[3].name).toBe("Retained earnings");
  ...
  describe("the notes", () => {
    it("numbers the notes and returns them all", async () => {
      const notes = await N();
      expect(notes.map((n) => n.number)).toEqual([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    ...
    it("presents the shareholder current account as the balance that is related by construction", async () => {
      ...
      // implying that this is the whole of it.
  ...
  describe("statement and notes together", () => {
    it("defaults to the most recent fiscal year the ledger holds", async () => {
      ...
      expect(both.notes).toHaveLength(9);
